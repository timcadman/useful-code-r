...
# This makes a table of all items that don't reach the threshold

# Takes as arguments (i) fa object, (ii) cut-off threshold and (iii) variable 
# labels

faTabLoadExc <- function(object, labels, cutoff){
...
load <- subset(load_tmp, load_tmp$above == 1)
load <- round_df(load_above, digits = 2) #round to 2 digits
